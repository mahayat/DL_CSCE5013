...
def SGD(X,Y,W,B,LR):
    ...
    for i in np.arange(x.shape[0]):
        ...
        dLdW11 = X[i,0]*drelu(H1)*W[4]*drelu(H3)*(Y_hat-Y[i])    
        dLdW12 = X[i,0]*drelu(H2)*W[5]*drelu(H3)*(Y_hat-Y[i])
        
        dLdW21 = X[i,1]*drelu(H1)*W[4]*drelu(H3)*(Y_hat-Y[i])    
        dLdW22 = X[i,1]*drelu(H2)*W[5]*drelu(H3)*(Y_hat-Y[i])    

        dLdW31 = Z1*drelu(H3)*(Y_hat-Y[i])    
        dLdW32 = Z2*drelu(H3)*(Y_hat-Y[i])
    
        dLdB1 = drelu(H1)*W[4]*drelu(H3)*(Y_hat-Y[i])
        dLdB2 = drelu(H2)*W[5]*drelu(H3)*(Y_hat-Y[i])
        dLdB3 = drelu(H3)*(Y_hat-Y[i])
