...
def SGD(X,Y,W,B,LR):
    ...
    for i in np.arange(x.shape[0]):
        ...
        dLdW11 = X[i,0]*dsig(H1)*W[4]*dsig(H3)*dCE(Y_hat,Y[i])    
        dLdW12 = X[i,0]*dsig(H2)*W[5]*dsig(H3)*dCE(Y_hat,Y[i])
        
        dLdW21 = X[i,1]*dsig(H1)*W[4]*dsig(H3)*dCE(Y_hat,Y[i])   
        dLdW22 = X[i,1]*dsig(H2)*W[5]*dsig(H3)*dCE(Y_hat,Y[i])    

        dLdW31 = Z1*dsig(H3)*dCE(Y_hat,Y[i])   
        dLdW32 = Z2*dsig(H3)*dCE(Y_hat,Y[i])
    
        dLdB1 = dsig(H1)*W[4]*dsig(H3)*dCE(Y_hat,Y[i])
        dLdB2 = dsig(H2)*W[5]*dsig(H3)*dCE(Y_hat,Y[i])
        dLdB3 = dsig(H3)*dCE(Y_hat,Y[i])
