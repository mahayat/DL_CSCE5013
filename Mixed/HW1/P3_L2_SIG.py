...
def SGD(X,Y,W,B,LR):
    ...
    for i in np.arange(x.shape[0]):
        ...
        dLdW11 = X[i,0]*dsig(H1)*W[4]*dsig(H3)*(Y_hat-Y[i])    
        dLdW12 = X[i,0]*dsig(H2)*W[5]*dsig(H3)*(Y_hat-Y[i])
        
        dLdW21 = X[i,1]*dsig(H1)*W[4]*dsig(H3)*(Y_hat-Y[i])    
        dLdW22 = X[i,1]*dsig(H2)*W[5]*dsig(H3)*(Y_hat-Y[i])    
